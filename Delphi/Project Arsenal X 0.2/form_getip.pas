...
uses
  ...
  sButton, sEdit, sGroupBox, sSkinManager, functions;
...
procedure Tgetip.btnGetIPClick(Sender: TObject);
var
  ip: string;
begin
  ...
  begin
    ...
    form_getip.getip.status.Update;
    ip := get_ip(txtHost.Text);
    if not(ip = '') then
    begin
      txtResult.Text := ip;
    end
    ...
    status.Panels[0].Text := '[+] Finsihed';
    form_getip.getip.status.Update;
  end
  else
  begin
    MessageBox(0, 'Write Host', 'Get IP 0.2', MB_ICONINFORMATION);
  end;
end;
